...
// Etienne: Tweaked so that we pass a raw array and not a vector
bool PdBase::writeArray(const std::string& arrayName, float* source, int writeLen, int offset) {
    ...
    _LOCK();
    if(libpd_write_array(arrayName.c_str(), offset, source, writeLen) < 0) {
